...
public class EnemyAI : MonoBehaviour
{
    ...
    Vector3 CheckGround(Vector3 pos)
    {
        ...
        if (floorLeft.collider != null || floorMiddle.collider != null || floorRight.collider != null)
        {
            ...
            pos.y = hitRay.collider.bounds.center.y + hitRay.collider.bounds.size.y / 2f + .5f;
    ...
    {
        Vector2 originTop = new Vector2 (pos.x + direction *.4f, pos.y + .5f - 0.2f);
        Vector2 originMiddle = new Vector2(pos.x + direction * .4f, pos.y);
        Vector2 originBottom = new Vector2(pos.x + direction * .4f, pos.y - .5f + 0.2f);

        RaycastHit2D wallTop = Physics2D.Raycast(originTop, new Vector2(direction, 0f), velocity.x * Time.deltaTime, wallMask);
        RaycastHit2D wallMiddle = Physics2D.Raycast(originMiddle, new Vector2(direction, 0f), velocity.x * Time.deltaTime, wallMask);
        RaycastHit2D wallBottom = Physics2D.Raycast(originBottom, new Vector2(direction, 0f), velocity.x * Time.deltaTime, wallMask);
